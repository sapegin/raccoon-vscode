...
import { getRepositoryUrl } from './util/getRepositoryUrl';

/**
 * Color to use for entries that are older than the last available color in the
 * scale
 */
const NO_COLOR = '#fff0';

/**
 * Non-breaking space: has the correct width but doesn’t get squished like a
 * regular space
 */
const NBSP = '\u00A0';
/** Narrower space */
...
/**
 * Finds the element in the array that has the maximum value when passed through
...
function maxBy<T>(array: T[], iteratee: (item: T) => number): T | undefined {
  ...
  for (const element of array) {
    ...
    if (maxElement === undefined || value > maxValue) {
      maxValue = value;
      maxElement = element;
    }
  }
...
export class BlameManager {
  ...
  public async open(editor: TextEditor) {
    ...
    const repoUrl = await getRepositoryUrl(workspaceRoot);
    ...
    this.colorMap = sortedDistinctDates.reduce<Record<string, string>>(
      (accumulator, date, index) => {
        if (index < colorScale.length) {
          accumulator[date] = colorScale[index] ?? NO_COLOR;
        }
        return accumulator;
      },
      {}
